...
function Jostle:GetScreenTop()
	...
	for _,frame in ipairs(self.topFrames) do
		if frame.IsShown and frame:IsShown() and frame.GetBottom and frame:GetBottom() then --and frame:GetBottom() < bottom then
			-- added * frame:GetScale() by yess
			bottom = frame:GetBottom()  * frame:GetScale()
		end
	end
	return bottom
end
